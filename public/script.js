...
peer.on("open", async (id) => {
  cUser = id;
  await socket.emit("join-room", ROOM_ID, id, username);
});
...
const listOfUser = (userIdUsername) => {
  console.log("my user id and username", userIdUsername);
  ...
  for (var i = 0; i < userlist.length; i++) {
    ...
    var t = document.createTextNode(
      `VideoSector ${userIdUsername[i].username} ${i + 1}`
    );
    x.appendChild(t);
...
socket.on("all_users_inRoom", (userI, userIdUsername) => {
  console.log("UserI", userI);
  userlist.splice(0, userlist.length);
  userlist.push.apply(userlist, userI);
  console.log(userlist);
  listOfUser(userIdUsername);
